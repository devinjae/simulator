...
Revision ID: 6b9c6a7a2a2f
Revises: 
Create Date: 2025-10-11 00:08:28.000878
...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_macro_factors_name'), 'macro_factors', ['name'], unique=False)
    ...
    sa.Column('shock_type', sqlmodel.sql.sqltypes.AutoString(), nullable=False),
    sa.Column('scope', sqlmodel.sql.sqltypes.AutoString(), nullable=False),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_news_events_ts_release_ms'), 'news_events', ['ts_release_ms'], unique=False)
    ...
    op.create_table('instrument_factor_exposure',
    sa.Column('instrument_id', sa.Uuid(), nullable=False),
    sa.Column('factor_id', sa.Uuid(), nullable=False),
    sa.Column('beta', sa.Float(), nullable=False),
    sa.ForeignKeyConstraint(['factor_id'], ['macro_factors.id'], ),
    sa.ForeignKeyConstraint(['instrument_id'], ['instruments.id'], ),
    sa.PrimaryKeyConstraint('instrument_id', 'factor_id')
    )
    op.create_table('instrument_sector_exposure',
    sa.Column('instrument_id', sa.Uuid(), nullable=False),
    ...
    op.create_table('news_event_factors',
