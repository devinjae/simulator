...
class TestOrderBook(TestCase):
    def setUp(self):
        ...
        # Common test orders
        self.buy_order_100 = {"price": 100, "quantity": 10, "ticker": "AAPL", "user_id": "u1", "side": OrderSide.BUY}
        self.buy_order_101 = {"price": 101, "quantity": 5, "ticker": "AAPL", "user_id": "u2", "side": OrderSide.BUY}
        self.sell_order_102 = {"price": 102, "quantity": 8, "ticker": "AAPL", "user_id": "u3", "side": OrderSide.SELL}
        self.sell_order_103 = {"price": 103, "quantity": 12, "ticker": "AAPL", "user_id": "u4", "side": OrderSide.SELL}
    ...
    def test_order_insertion_middle(self):
        ...
        self.order_book.add_order({"price": 100, "quantity": 5, "ticker": "AAPL", "user_id": "u1", "side": OrderSide.BUY})
        self.order_book.add_order({"price": 105, "quantity": 5, "ticker": "AAPL", "user_id": "u2", "side": OrderSide.BUY})
        self.order_book.add_order({"price": 102, "quantity": 5, "ticker": "AAPL", "user_id": "u3", "side": OrderSide.BUY})
    ...
    def test_order_matching_partial(self):
        ...
        buy_order = {"price": 103, "quantity": 15, "ticker": "AAPL", "user_id": "u1", "side": OrderSide.BUY}
        ...
        self.assertEqual(len(self.order_book.sells), 0)  # Sell order should be fully matched and removed
        self.assertEqual(len(self.order_book.buys), 1)  # Buy order should be added with remaining quantity
    ...
    def test_order_matching_full(self):
        ...
        buy_order = {"price": 103, "quantity": 5, "ticker": "AAPL", "user_id": "u1", "side": OrderSide.BUY}
        ...
        self.assertEqual(len(self.order_book.sells), 1)  # Sell order should still exist with reduced quantity
        self.assertEqual(len(self.order_book.buys), 0)  # Buy order should be fully matched and not added
    ...
    def test_order_matching_no_match(self):
        ...
        buy_order = {"price": 100, "quantity": 5, "ticker": "AAPL", "user_id": "u1", "side": OrderSide.BUY}
        ...
        self.assertEqual(len(self.order_book.buys), 1)  # Buy order should be added to the book
        self.assertEqual(len(self.order_book.sells), 1)  # Sell order should remain unchanged
    ...
    def test_mid_price_calculation(self):
        ...
        # Add a buy order
        self.order_book.add_order(self.buy_order_100.copy())
        self.assertIsNone(self.order_book.mid_price())  # Still None because we need both buy and sell orders
    ...
    def test_multiple_order_matching(self):
        ...
        self.order_book.add_order({"price": 101, "quantity": 3, "ticker": "AAPL", "user_id": "u1", "side": OrderSide.SELL})
        self.order_book.add_order({"price": 102, "quantity": 5, "ticker": "AAPL", "user_id": "u2", "side": OrderSide.SELL})
        
        # Create a large buy order that will match both sell orders
        buy_order = {"price": 103, "quantity": 7, "ticker": "AAPL", "user_id": "u3", "side": OrderSide.BUY}
        ...
        # Verify the results
        self.assertEqual(status, OrderStatus.PARTIALLY_FILLED)
        self.assertEqual(remaining_qty, 1)  # 7 - (3 + 5) = -1, but should be 0 if we're capping at available
        self.assertEqual(len(self.order_book.sells), 1)  # One sell order should be fully matched, one partially
        self.assertEqual(self.order_book.sells[0]["quantity"], 5)  # The second sell order should be partially filled